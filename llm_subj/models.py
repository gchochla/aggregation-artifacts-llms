...
from llm_subj.base_prompts import LabelSimilarityMixin
    ...
    def forward(
        ...
        **kwargs,
        ...
        out_dict = self.lm.generate(
            **kwargs, return_dict_in_generate=True, output_scores=True
        )
        ...
        if cutoff_str is not None and self.tokenizer is not None:
            ...
            ids_cutoff_inds = [
                self._tensor_overlap(o, c.to(o.device))
                for o, c in zip(orig_ids, cutoff_ids)
            ]
            out["scores"] = [
                o[:i] for o, i in zip(out["scores"], ids_cutoff_inds)
            ]
            ...
            assert all(
                ...
                "IDs and scores must have the same length, but got "
                f"{[len(o) for o in out['ids']]} and {[len(s) for s in out['scores']]}"
            )
        ...
        if prefix_cutoff_str is not None and self.tokenizer is not None:
            ...
            ids_prefix_cutoff_inds = [
                self._tensor_overlap(o, c.to(o.device))
                for o, c in zip(orig_ids, prefix_cutoff_ids)
            ]
            out["scores"] = [
                o[i:] for o, i in zip(out["scores"], ids_prefix_cutoff_inds)
            ]
            assert all(
                len(ids) == len(scores)
                for ids, scores in zip(out["ids"], out["scores"])
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.label_first_token_ids is not None:
            ...
            # find the index of the first token that is a label in out["ids"]
            first_label_inds = [
                min(
                    [
                        self._tensor_overlap(o, torch.tensor([i]).to(o.device))
